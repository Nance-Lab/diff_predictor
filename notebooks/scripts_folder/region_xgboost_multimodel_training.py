...
#     rand_integers = random.sample(set(np.arange(0, len(filelist))), 6)
#     for rand_int in rand_integers:
#         sampled_filelist.append(filelist[rand_int])
# fstats_tot = generate_fullstats(datasetpath, sampled_filelist, ['cortex', 'ganglia', 'hippocampus', 'striatum', 'thalamus'], 'region')
...
# features_df = sampled_df.drop(['region', 'X', 'Y', 'binx', 'biny', 'bins', 'Track_ID'], axis=1)
...
best_param = {'max_depth': 10, 'eta': 0.005, 'min_child_weight': 0, 'verbosity': 0, 'objective': 'multi:softprob', 'num_class': 5, 'silent': 'True', 'gamma': 5, 'subsample': 1.0, 'colsample_bytree': 0.5, 'eval_metric': 'mlogloss', 'gpu_id': 0, 'tree_method': 'gpu_hist', 'predictor': 'gpu_predictor'}
...
for i in range(500):
    ...
    dtest = xgb.DMatrix(X_test[features], label=y_test)
    dval = xgb.DMatrix(X_val[features], label=y_val)
    booster, acc, true_label, preds = train(param, dtrain, dtest, dval, evals=[(dtrain, 'train'), (dval, 'eval')], num_round=750)
    acc_list.append(acc)
...
output_dict = {'Accuracies': acc_list, 'True Labels': true_label_list, 'Preds': preds_list, 'Trajectory Count': traj_count_list, 'Frames': frames_list}
