...
features_df = sampled_df.drop([target, 'X', 'Y', 'binx', 'biny', 'bins', 'Track_ID', 'Filename'], axis=1)
...
for file_name in filelist:
    ...
    #model_list = [] #too expensive to store all models
    ...
    #     break
        ...
        print(f' data_point_preds_shape: {len(data_point_preds)}')
        true_label_list = [true_label[0]]*len(data_point_preds)
        print(true_label_list)
        print(true_label[0:len(data_point_preds)])
        acc = accuracy_score(y_pred=data_point_preds, y_true=true_label_list)
        #print(acc)
